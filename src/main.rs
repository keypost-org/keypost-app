...
fn login_start(payload: Json<LoginStart>) -> JsonValue {
    ...
    let nonce = rand::random::<u32>();
    let server_login_bytes = server_login_start_result.state.serialize().expect("Could not get bytes!");
    cache::insert(nonce, server_login_bytes);
    let response_bytes = server_login_start_result.message.serialize().expect("Could not get bytes!");
    let response = base64::encode(response_bytes);
    json!({ "id": &nonce, "o": &response })
}
